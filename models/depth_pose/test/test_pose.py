...
def load_image_sequence(dataset_dir, 
    ...
    for o in range(-half_offset, half_offset+1):
        ...
        img_file=os.path.join(dataset_dir+"/1-s",'%s.png'%(curr_frame_id))
...
def main():
    ...
    img_dir = os.path.join(FLAGS.dataset_dir, '1-s')
    N = len(glob(img_dir + '/*.png')[:4540])
    test_frames = ['%.2d %.7d' % (FLAGS.test_seq, n) for n in range(N)]
